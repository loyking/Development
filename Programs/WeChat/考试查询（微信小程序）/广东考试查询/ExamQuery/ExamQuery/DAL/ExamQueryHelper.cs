...
namespace ExamQuery.DAL
{
    public class ExamQueryHelper
    {
        ...
        public static void InsertStudentInfo(string StudentName,string Sfz,string Zkzh,int ProvinceKey,int CityKey)
        {
            if (!IsRepeat(Sfz, Zkzh))
            {
                string sql = "insert into StudentInfo values(@StudentName,@Sfz,@Zkzh,@ProvinceKey,@CityKey)";
                SqlParameter[] parameters = {
                new SqlParameter("@StudentName",StudentName),
                new SqlParameter("@Sfz",Sfz),
                new SqlParameter("@Zkzh",Zkzh),
                new SqlParameter("@ProvinceKey",ProvinceKey),
                new SqlParameter("@CityKey",CityKey)
                };
        ...
        public static void InsertZkScore(string Zkzh, string Sfz,ZkScoreResult zk)
        {
            ...
            if ((int)SQLHelper.ExcuteScalar(exits, sqls) == 0)
            {
                string sql = "insert into ZkScore values(@StudentId,@ChineseScore,@MathematicsScore,@EnglishScore,@HistoryScore,@GeographyScore,@BiologyScore,@ChemistryScore,@PhysicsScore,@PoliticsScore,@SumScore)";
                    SqlParameter[] parameters = {
                    new SqlParameter("@StudentId",Id),
        ...
        public static void InsertZkAdmissions(string Zkzh, string Sfz,ZkAdmissionsResult zk)
        {
            ...
            if ((int)SQLHelper.ExcuteScalar(exits, sqls) == 0)
            {
                string sql = "insert into ZkAdmissions values(@StudentId,@SchoolName)";
                SqlParameter[] parameters = {
                new SqlParameter("@StudentId",Id),
