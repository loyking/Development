...
namespace T.Controllers
{
    public class HomeController : Controller
    {
        ...
        {
            LoginWx(new object());
            Thread thd = new Thread(new ParameterizedThreadStart(GetGuanZhouF));    //需要执行的方法（该方法必须有一个参数为object的重载）
        ...
        private void GetGuanZhouF(object obj)
        {
            WebBrowser web = GetPage("http://www.gz5zx.com/Portal/Index/Main_SchoolProfilesss");
            HtmlDocument doc = web.Document;

            //获取页面中的ul列表
            HtmlElementCollection ulList;
            foreach (HtmlElement item in doc.GetElementsByTagName("div"))
            {
                if (item.GetAttribute("class") == "kuang3")
                {
                    ulList = item.GetElementsByTagName("li");
                }
                else
                {
                    continue;
